import { ADD_ENTRY, UPDATE_ENTRY, DELETE_ENTRY } from './definitions';

const entries = (state = {}, action) => {
  switch (action.type) {
    case ADD_ENTRY:
      const newEntryList = state[action.listId] && state[action.listId].slice() || [];
      newEntryList.push(action.entry);
      const newState = {};
      newState[action.listId] = newEntryList;
      return Object.assign({}, state, newState);
    case UPDATE_ENTRY:
      const newEntry = {};
      if (action.entry.ranking) newEntry.ranking = action.entry.ranking;
      if (action.entry.text) newEntry.text = action.entry.text;
      const newList = state[action.listId] && state[action.listId].slice() || [];
      const i = newList.findIndex(entry => entry.id === action.entry.id);
      newList.splice(i, 1, newEntry);
      const newState = {};
      newState[action.listId] = newList;
      return Object.assign({}, state, newState);
    case DELETE_ENTRY:
